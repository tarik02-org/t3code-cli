...
} from "./environment-name.ts";
import { expectConfigError } from "../test/helpers/assert-errors.ts";
...
describe("defaultEnvironmentNameFromUrl", () => {
  ...
  it.effect("maps invalid urls to ConfigError", () =>
    expectConfigError(defaultEnvironmentNameFromUrl("not-a-url"), "invalid url"),
  );
});
